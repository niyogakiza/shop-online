...
class ProductContainer extends Component{
    ...
    filterHighestPrice(arr){
        return arr.length <= 1 ? arr :
            this.filterHighestPrice(arr.slice(1).filter(
                item => item.price.salePrice >= arr[0].price.salePrice
            )).concat(arr[0],
                this.filterHighestPrice(arr.slice[1].filter(
                    item => item.price.salePrice < arr[0].price.salePrice
                )))
    ...
    filterLowestPrice(arr){
        return arr.length <= 1 ? arr : this.filterLowestPrice(arr.slice[1].filter(
            item => item.price.salePrice <= arr[0].price.salePrice
        )).concat(arr[0],
            this.filterLowestPrice(arr.slice[1].filter(
                item => item.price.salePrice > arr[0].price.salePrice
            )))
    }

    render(){
        const {dataProducts, showcaseHeader } = this.props;
        const { filter } = this.state;
        const { handleSelectSort } = this;
        ...
        const currentPage = this.props.location.hash ?
            this.props.location.hash.substr(1) * 1 : 1;
