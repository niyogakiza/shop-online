...
const propTypes = { handleAddToCart: PT.func };

class ProductDetailSite extends Component{
    constructor(props){
        super(props);

        this.state = { dataAvisory: [], dataComments: []}
    }

    componentDidMount(){
        axios
            .get('/comment', {dataType: 'json'})
            .then(res => this.setState({ dataComments: res.data.data }))
